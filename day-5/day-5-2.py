...
class IntCodeComputer:
    ...
    JMP_OPS = [OPS['JIFT'], OPS['JIFF']]

    def __init__(self, tape, inputstream, outputstream):
        self.position = 0
        self.tape = tape
        self.funcs = [
            noop, add, mult, self.input, self.output, self.jift, self.jiff, lt,
            eq]
        self.halted = False
    ...
    def writeback(self):
        if self.op < 3 or self.op in [
                IntCodeComputer.OPS['LT'], IntCodeComputer.OPS['EQ']]:
    ...
    def get_val_p(self):
        ...
        if self.op in IntCodeComputer.JMP_OPS:
            return 3
    ...
    def jift(self, val1, val2, *args):
        if val1 != 0:
            self.val_p = 0
            self.position = val2

    def jiff(self, val1, val2, *args):
        if val1 == 0:
            self.val_p = 0
            self.position = val2
