...
execpos op pos tape =
    ...
    in  take outpos tape ++ (
            op (tape !! arg1) (tape !! arg2) : drop (outpos + 1) tape)
...
findsoln tape noun verb
    | attempt == target = 100 * noun + verb
    | otherwise         = 0
    where (attempt : _) = exec 0 (1 : noun : verb : drop 3 input)

main =
    let combos     = [ (x, y) | x <- [0..99], y <- [0..99] ]
        (soln : _) = dropWhile (==0) (
                         map (\x -> findsoln input (fst x) (snd x)) combos)
    in  print $ soln
